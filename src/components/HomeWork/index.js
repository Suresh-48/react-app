...
class HomeWork extends Component {
  ...
  updateAnswer = (answer) => {
    ...
    Api.post("api/v1/homeworkSchedule/student/answer", {
      homeworkId: this.state.homeworkScheduleId,
      answers: answer,
  ...
  handleSubmit = async () => {
    ...
    if (fileUploadCount.length === 0) {
      ...
      Api.post("api/v1/homeworkSchedule/file/upload", formData, { headers: { token: token } })
        .then((res) => {
  ...
  render() {
    const { lessonNumber, courseName, lessonName } = this.state;
    return (
      <div>
        <Container>
          <Row>
