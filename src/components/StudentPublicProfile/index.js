...
function StudentPublicProfile(props) {
  ...
  const userDetails = () => {
    Api.get(`api/v1/student/${props.studentId}`, { headers: { token: token } }).then((response) => {
      const data = response.data.data.getOne;
      setUserDetail(data).catch((error) => {
                ...
                <Col xs={12} sm={6} xd={6}>
                  <Card className="popup-card-shadow" style={{ marginTop: 20 }}>
                    <div style={{ textAlign: "center" }}>
                      ...
                      <p className="popup-count-style">{cardDetail ? cardDetail?.activeCourse : cardDetail}</p>
                    </div>
                  </Card>
                </Col>
                <Col xs={12} sm={6} xd={6}>
                  <Card className="popup-card-shadow" style={{ marginTop: 20 }}>
                    <div style={{ textAlign: "center" }}>
                      ...
                      <p className="popup-count-style">{cardDetail ? cardDetail?.completeCourse : cardDetail}</p>
                    </div>
                  </Card>
                </Col>
              </Row>
